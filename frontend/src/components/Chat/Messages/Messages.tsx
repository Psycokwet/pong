  ...
  const filteredMessages: Message[] = messages.filter((message) => {
    return (
      blockedUserList.find(
        (blockedUser) => blockedUser.pongUsername == message.author
      ) == undefined
    );
  });
  const getUrl = (message: Message) => {
    const found = avatarList.find(
      (avatar) => avatar.pongUsername == message.author
    );
    if (found) return found.avatarUrl;
    return "";
  };
  return (
    <div className="flex flex-col-reverse row-span-5 col-span-3 scroll-smooth overflow-y-auto">
